...
class Node{
    constructor(key){
        ...
        this.left = null;
        this.right = null;
        vetHelp.push(key);// esté vetor foi feita para ajudar a calibrar a árvore
    }
}
    ...
    search(valor){
        ...
        if(valor < raiz.key){
            ...
            if(raiz.key == valor){
                ...
                let raizR = raiz.right;
                raiz = raiz.left;

                if(raiz.key == valor || raizR.key == valor){
                    return true;
                } else {
                    raizR = raiz.right;
                    raiz = raiz.left;

                    if(raiz == null || raizL == null) {
                        return false;
                    } else if(raiz.key == valor || raizR.key == valor){
                        return true;
                    }
        ...
        if(valor > raiz.key){
            ...
            if(raiz.key == valor){
                ...
                if(raiz == null || raizL == null){
                    return false;
                } else if(raiz.key == valor || raizL.key == valor){
                    return true;
                } else {
                    raizL = raiz.left;
                    raiz = raiz.right;
...
function addValor(){
    ...
    if(isNaN(val)){//está conferindo se é número
                document.getElementById("feedback").innerHTML = "somente números são aceitos";                
    }else{
        if(valor == "" || valor == null ){
            document.getElementById("feedback").innerHTML=  "Adicione um número";
              ...
              document.getElementById("entrada").focus();
              tree.print();
              document.getElementById("feedback").innerHTML=  "O Valor <b>" + valor + "</b> foi adicionado com Sucesso!";              
          }            
    }//Fim conferir se pe número
...
function valorMaximo(){
    ...
    let max = Math.max(...vetHelp);
    if(vetHelp.length == 0){
        document.getElementById("feedback").innerHTML=  "Árvore está Vazia!";
        document.getElementById("entrada").value = "";
        document.getElementById("entrada").focus();
    }
    else {       
      document.getElementById("feedback").innerHTML=  "O Valor Minimo da Arvore é <b>" +  max  + "</b>";
    }
}
function valorMinimo(){
   
    let min = Math.min(...vetHelp);
    if(vetHelp.length == 0){
        document.getElementById("feedback").innerHTML=  "Árvore está Vazia!";
        document.getElementById("entrada").value = "";
        document.getElementById("entrada").focus();
      ...
      document.getElementById("feedback").innerHTML=  "A Arvore está vazia!";
    } else {
       
      document.getElementById("feedback").innerHTML= "o Valor Minimo da Arvore é <b> " +  min  + "</b>";
    }
}
//função para balancear á árvore
function calibrar(){
    //tree.getClear();
    
    let meio = (vetHelp.length/2).toFixed(0) - 1;//para deixar o numero inteiro
    let vetBalancear = vetHelp.sort(function(a, b){return a - b});
    vetHelp = [];
    
   tree.getClear();
   tree.inserir(vetBalancear[meio]);
   count = 2;
    for(let i = meio+1; i < vetBalancear.length  ; i++){
       
            if(count == 2){
                if(vetBalancear[i+1] === undefined){
                    tree.inserir(vetBalancear[i]);
                }else{
                    tree.inserir(vetBalancear[i+1]);
                    count = 1;
                }
            }else if(count == 1){
                if(vetBalancear[i+1] === undefined){
    ...
    count = 2;
    for(let i = meio; i > 0 ; i--){
        if(i-1 < 0){
            tree.inserir(vetBalancear[1]);
        }else{
            if(count == 2){
                tree.inserir(vetBalancear[i+1]);
                count = 1;
    ...
    //gabiarra para dar certo
    tree.inserir(vetBalancear[1]);
    tree.inserir(vetBalancear[0]);
    tree.inserir(vetBalancear[vetBalancear.length - 2]);
    tree.inserir(vetBalancear[vetBalancear.length - 1]);
    tree.print();
    
}
//função para balancear á árvore

//para adiconar valores com enter
var input = document.getElementById("entrada");
input.addEventListener("keyup", function(event) {
    if (event.keyCode === 13) {
        event.preventDefault();
        document.getElementById("addValor").click();
    }
});
...
function encontrarValor(){
    ...
    if(valor == "" || valor == null){
      document.getElementById("feedback").innerHTML=  "Adicione um valor para ser procurado!";
        document.getElementById("entrada").focus();
    } else {
        if(vetHelp.length == 0){
            document.getElementById("feedback").innerHTML=  "Árvore está Vazia!";
        ...
        else if( vetHelp.find(contem)){
          document.getElementById("feedback").innerHTML=  "O Valor <b>" + valor + "</b> está na árvore!";
            document.getElementById("entrada").value = "";
            document.getElementById("entrada").focus();
        } else {
          document.getElementById("feedback").innerHTML=  "O Valor Não <b>" + valor + "</b> está na árvore!";
            document.getElementById("entrada").value = "";
            document.getElementById("entrada").focus();
        }
    }
