...
class Lista{
    ...
    push(element){
        ...
        if(this.head == null){
           ...
           // document.getElementById("feedback").innerHTML= 'O elemento: "' + this.head.value + '" Foi adicionado com sucesso!' ;
            ...
            document.getElementById("feedback").innerHTML=  'O elemento: "' + element + '" já existe não pode ser adicionado!';
    ...
    insertAt(element, position){
        ...
        }else if(position >= 0 && position <= this.count && this.noRepeat(element) == false){
            const node = new Node(element);
            if(position === 0 ){
...
function printFila(item, index){
    ...
    let node = document.createElement("p");//1
